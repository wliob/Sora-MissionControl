  ...
  return (
    <>
      {USAGE_METRIC_SEEDS.map((seed) => {
        ...
        const freshness = metric?.provenance.freshness ?? 'unknown';
        const confidence = metric?.provenance.confidence ?? 'unknown';
    ...
    return (
      <div style={{ color: 'var(--text-dim)', fontSize: 'var(--text-xs)' }}>
        ...
        <div className="mono" style={{ marginTop: 'var(--space-1)', color: 'var(--text-dim)' }}>
          {provenance.source} • {provenance.freshness}/{provenance.confidence}
      ...
      {quotas.map((quota: ProviderQuotaSnapshot) => (
        ...
        >
          <div>
            ...
            >
              {quota.provenance.source} • {quota.provenance.freshness}/{quota.provenance.confidence}
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ flexShrink: 0, borderBottom: '1px solid var(--border-faint)' }}>
        ...
        <div>
          {buildSourceRows(sources).map((source) => {
